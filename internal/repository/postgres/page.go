...
func (r *PageRepository) CreatePage(ctx context.Context, spacePkID int64, name string, viewType string, ParentPagePkID *int64, nodeIDInput string) (*domain.Page, *domain.Error) {
	nodeID := nodeIDInput
	if nodeID == "" {
	...
	return pageutils.MapPageModelToDomain(newPage, nil), nil
}
...
func (r *PageRepository) GetPagesBySpacePkID(ctx context.Context, spacePkID int64, excludeArchived bool) ([]domain.Page, *domain.Error) {
	...
	for _, page := range pages {
		domainPages = append(domainPages, *pageutils.MapPageModelToDomain(page, nil))
	}
...
func (r *PageRepository) DeletePageByPkID(ctx context.Context, pagePkID int64, userPkID int64) (*domain.Page, *domain.Error) {
	...
	return pageutils.MapPageModelToDomain(page, nil), nil
}
...
func (r *PageRepository) GetPageByID(ctx context.Context, pageID string) (*domain.Page, *domain.Error) {
	...
	for _, childPage := range childPages {
		childPagesDomain = append(childPagesDomain, *pageutils.MapPageModelToDomain(childPage, nil))
	}

	return pageutils.MapPageModelToDomain(page, childPagesDomain), nil
}
